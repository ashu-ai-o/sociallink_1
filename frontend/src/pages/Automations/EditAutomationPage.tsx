// ============================================================================
// Edit Automation PAGE - Clean step-by-step wizard with ZapDM-inspired design
// ============================================================================
...
export const EditAutomationPage = () => {
  ...
  const automation = useAppSelector((state) =>
    state.automations.items.find((a) => a.id === id)
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg = typeof error === 'string' ? error : error?.message || error?.response?.data?.detail || 'Failed to Edit Automation';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
...
const Step4 = ({ formData }: any) => (
  <div className="space-y-6">
    <div>
      ...
      <p className="text-neutral-600 dark:text-neutral-400">
        Review your automation settings before saving changes
      </p>
    </div>
